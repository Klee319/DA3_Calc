...
describe('damageCalculator', () => {
  ...
  describe('applyCritDamage', () => {
    it('会心100%モードで2倍のダメージを返す', () => {
      const result = applyCritDamage(1000, 25, 'always');
      expect(result.damage).toBe(2000);
      expect(result.multiplier).toBe(2);
    });

    it('会心0%モードで基礎ダメージをそのまま返す', () => {
      const result = applyCritDamage(1000, 25, 'never');
      expect(result.damage).toBe(1000);
      expect(result.multiplier).toBe(1);
    });

    it('期待値モードで会心率を考慮したダメージを返す', () => {
      const result = applyCritDamage(1000, 50, 'expected');
      expect(result.damage).toBe(1500); // 1000 * (1 + 0.5)
  ...
  describe('applyDamageCorrection', () => {
    it('minモードで最小値を返す', () => {
      const correction = applyDamageCorrection(80, 'min');
      expect(correction).toBeCloseTo(0.8);
    });

    it('maxモードで最大値（1.0）を返す', () => {
      const correction = applyDamageCorrection(80, 'max');
      expect(correction).toBe(1.0);
    });

    it('avgモードで平均値を返す', () => {
      const correction = applyDamageCorrection(80, 'avg');
      expect(correction).toBeCloseTo(0.9); // (0.8 + 1.0) / 2
    });
  });
  ...
  describe('calculateFinalDamage', () => {
    it('敵防御と耐性を適用できる', () => {
      ...
      const finalDamage = calculateFinalDamage(1000, enemy);
      // (1000 - 100/2) * 0.8 * 0.9 = 950 * 0.8 * 0.9 = 684
      expect(finalDamage).toBe(684);
    });
