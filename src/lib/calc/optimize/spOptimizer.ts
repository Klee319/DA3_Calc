...
export function optimizeRemainingSP(
  ...
  let bestAlloc: Record<string, number> = {};
    ...
    for (const b of optB) {
      ...
      for (const c of optC) {
        ...
        if (score > bestScore) {
          bestScore = score;
          bestAlloc = {};
          if (branchNames[0]) bestAlloc[branchNames[0]] = a.sp;
          if (branchNames[1]) bestAlloc[branchNames[1]] = b.sp;
          if (branchNames[2]) bestAlloc[branchNames[2]] = c.sp;
        }
      }
    }
  }

  const totalUsed = Object.values(bestAlloc).reduce((sum, v) => sum + (v || 0), 0);
