...
/**
 * 丸めタイプ
 * @see spec-skill-damage.md §6
 */
export type RoundType = 'up' | 'down' | 'nearest';

/**
 * 指定された丸めタイプで丸める
 * spec-skill-damage.md §6 に基づく実装
 *
 * @param value 値
 * @param roundType 丸めタイプ（デフォルト: nearest）
 * @returns 丸めた値
 */
export function roundByType(value: number, roundType: RoundType = 'nearest'): number {
  switch (roundType) {
    case 'up':
      return Math.ceil(value);
    case 'down':
 ...
 * spec-skill-damage.md §6 に基づく実装
 *
 * @param damage 1hit当たりのダメージ
 * @param hitCount hit数
 * @param roundPerHit hit毎に丸めるかどうか（デフォルト: true）
 * @param roundType 丸めタイプ
 * @returns 総ダメージ
 */
export function roundMultiHitDamage(
  damage: number,
  hitCount: number,
  roundPerHit: boolean = true,
  roundType: RoundType = 'nearest'
): number {
  if (roundPerHit) {
    // Hit毎に丸める
    const roundedDamagePerHit = roundByType(damage, roundType);
    return roundedDamagePerHit * hitCount;
  } else {
    // 最後にのみ丸める
    const totalDamage = damage * hitCount;
    return roundByType(totalDamage, roundType);
  }
}
